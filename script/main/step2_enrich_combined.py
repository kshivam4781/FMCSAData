...
def extract_all_detail_fields(detail_data):
    """Extract all enrichment fields from detail response:
    - STATUS_DETAIL
    ...
    # Single pass through DRAWER_DETAIL_LIST to extract all fields
    for item in detail_data['DRAWER_DETAIL_LIST']:
        ...
        # Extract Statement of Info Due Date (multiple label variations)
...
def has_all_enrichment_data(record):
    """Check if record already has all enrichment fields populated"""
    status_detail = record.get('STATUS_DETAIL')
    soi_due_date = record.get('STATEMENT_OF_INFO_DUE_DATE')
    principal_addr = record.get('PRINCIPAL_ADDRESS')
    mailing_addr = record.get('MAILING_ADDRESS')
    
    # Check if all fields exist and are not empty/None
    has_status = status_detail and str(status_detail).strip() and str(status_detail).lower() != 'none'
    has_soi = soi_due_date and str(soi_due_date).strip() and str(soi_due_date).lower() != 'none'
    has_principal = principal_addr and str(principal_addr).strip() and str(principal_addr).lower() != 'none'
    has_mailing = mailing_addr and str(mailing_addr).strip() and str(mailing_addr).lower() != 'none'
    
    return has_status and has_soi and has_principal and has_mailing

# Main execution
print("=" * 80)
print("STEP 2: ENRICH RECORDS WITH DETAILED INFORMATION")
print("=" * 80)
print()
print("This step will enrich records with:")
print("  - Status Detail")
print("  - Statement of Info Due Date")
...
print()
print("=" * 80)
print("CHECKING EXISTING ENRICHMENT DATA")
print("=" * 80)

# Check which records already have all enrichment data
...
for record_key, record in all_records.items():
    ...
    # Check if record already has all enrichment fields
    if has_all_enrichment_data(record):
...
print(f"Already fully enriched: {already_enriched}")
print(f"Need enrichment: {records_to_enrich}")
print()

if records_to_enrich == 0:
    print("All records already have complete enrichment data. No enrichment needed.")
    print("=" * 80)
    exit()
...
print("=" * 80)
print(f"Enriching {records_to_enrich} records with all detail fields...")
print()
...
print(f"  Already fully enriched: {stats['already_enriched']}")
