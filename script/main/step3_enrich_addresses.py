...
def extract_detail_fields(detail_data):
    """Extract Principal Address and Mailing Address from detail response"""
    ...
    for item in detail_data['DRAWER_DETAIL_LIST']:
        ...
        if label == 'Principal Address':
            principal_address = item.get('VALUE')
        elif label == 'Mailing Address':
            mailing_address = item.get('VALUE')
...
# Main execution
print("=" * 80)
print("STEP 3: ENRICH RECORDS WITH ADDRESSES")
print("=" * 80)
print()
...
print()
print("=" * 80)
print("CHECKING EXISTING ADDRESS DATA")
print("=" * 80)

# Check which records already have addresses
...
for record_key, record in all_records.items():
    ...
    # Check if record already has both addresses
    principal_addr = record.get('PRINCIPAL_ADDRESS')
    mailing_addr = record.get('MAILING_ADDRESS')
    if (principal_addr and str(principal_addr).strip() and str(principal_addr).lower() != 'none') and \
       (mailing_addr and str(mailing_addr).strip() and str(mailing_addr).lower() != 'none'):
...
print(f"Already have both addresses: {already_enriched}")
print(f"Need enrichment: {records_to_enrich}")
print()

if records_to_enrich == 0:
    print("All records already have addresses. No enrichment needed.")
    print("=" * 80)
    exit()

print("=" * 80)
print("ENRICHING RECORDS WITH ADDRESS INFORMATION")
print("=" * 80)
print(f"Enriching {records_to_enrich} records with Principal Address and Mailing Address...")
print()
...
print(f"  Already had addresses: {stats['already_enriched']}")
...
if 'records' in data:
    ...
    data['address_enrichment_date'] = time.strftime("%Y-%m-%d %H:%M:%S")
    data['address_enrichment_stats'] = stats
else:
    # If original was just a dict, create new structure
    data = {
        "extraction_date": data.get('extraction_date', 'Unknown'),
        "enrichment_date": data.get('enrichment_date', 'Unknown'),
        "address_enrichment_date": time.strftime("%Y-%m-%d %H:%M:%S"),
        "statistics": data.get('statistics', {}),
        "enrichment_stats": data.get('enrichment_stats', {}),
        "address_enrichment_stats": stats,
...
excel_file = f"Final_Addresses_{datetimestamp}.xlsx"
